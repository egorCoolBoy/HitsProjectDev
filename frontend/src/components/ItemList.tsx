...
import type { OrderItem, Participant } from '../app/App';
...
export function ItemList({ items, participants, onUpdateParticipantPortion, onSplitEvenly, onDeleteItem, isClosed = false }: ItemListProps) {
  ...
  return (
    <div className="space-y-3">
      {items.map((item) => {
        ...
        const isValidTotal = Math.abs(totalPortions - 1) < 0.01;
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {participants.map((participant) => {
                      const participantData = item.participants.find((p) => p.participantId === participant.id);
                      const portion = participantData?.portion || 0;
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              step="0.01"
                              value={portion}
                              onChange={(e) => handlePortionChange(item.id, participant.id, e.target.value)}
                              disabled={isClosed}
                  ...
                  <div className="mt-3 pt-3 border-t border-gray-100">
                    <div className={`flex justify-between text-xs ${isValidTotal ? 'text-green-600' : 'text-red-600'}`}>
                    ...
                    {!isValidTotal && totalPortions > 0 && (
                      <p className="text-xs text-red-600 mt-1">
                        Сумма частей должна быть равна 1
                      </p>
                    )}
                  </div>
                </div>
